...
class AgentPPO:
    ...
    # CPU array to GPU tensor to CPU array
    def select_action(self, states, if_explore=False):
        if if_explore:
    ...
    def update_policy_mp(self, batch_size, repeat_times, buffer_total):
        ...
        for buffer in buffer_total:
            ...
            a += buffer[3]
            log += buffer[4]
        tran = namedtuple('Transition', ('reward', 'mask',
        ...
        all__delta = torch.zeros(
            max_memo, dtype=torch.float32, device=self.device)
        all__old_v = torch.zeros(
            max_memo, dtype=torch.float32, device=self.device)  # old policy value
        all__adv_v = torch.zeros(
            max_memo, dtype=torch.float32, device=self.device)  # advantage value
        ...
        for i in range(max_memo - 1, -1, -1):
            """
            advantage：V(s') * gamma + r - V(s)
            """
            # 所谓的Advantage
            all__delta[i] = all_reward[i] + \
                all_mask[i] * prev_new_v - all__new_v[i]
            # 所谓的Reward to go
            all__old_v[i] = all_reward[i] + all_mask[i] * prev_old_v
            # 累积的advantage
            all__adv_v[i] = all__delta[i] + \
                all_mask[i] * prev_adv_v * lambda_adv
