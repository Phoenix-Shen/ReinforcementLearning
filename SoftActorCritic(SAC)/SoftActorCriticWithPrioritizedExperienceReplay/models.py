import datetime
from memory import ProritizedExperienceReplay
import torch as t
...
class Agent(nn.Module):
    ...
    def __init__(self,
        ...
        # replay buffer
        self.memory = ProritizedExperienceReplay(
            self.buffer_size, self.batch_size, self.clipped_abs_error, self.epsilon, self.mem_alpha, self.mem_beta, self.beta_increment)
    ...
    def _update_network(self):
        ...
        #  sample data   #
        ##################
        b_idx, obses, actions, rewards, obses_, dones, ISWeights = self.memory.sample_buffer(
        )
        # to_tensor
        ...
        ISWeights = t.FloatTensor(ISWeights).to(self.actor.device)
        ...
        tderr1 = target_qvalue.detach()-q1_val
        tderr2 = target_qvalue.detach()-q2_val
        loss_c1 = 0.5*(t.pow(tderr1, 2)*ISWeights).mean()
        loss_c2 = 0.5*(t.pow(tderr2, 2)*ISWeights).mean()
        ...
        self.memory.batch_update(b_idx, prios.detach().cpu().numpy())
        ###############################
